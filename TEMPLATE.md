...
This repo is registered as a GitHub template repository (Settings → Template repository). It ships as a valid, buildable, passing-lint connector — every file works as-is under the name `dpuse-connector-microsoft-onedrive`. Delete this file once you're done renaming it into your new connector.

## 1. Generate a new repo from this template

```bash
gh repo create dpuse/dpuse-connector-<name> --public --template dpuse/dpuse-connector-microsoft-onedrive
...
## 2. Rename `dpuse-connector-microsoft-onedrive` to your connector's id

The literal string `dpuse-connector-microsoft-onedrive` appears in `package.json` (name, bugs.url, repository.url, module/exports filenames), `config.json` (id), `README.md`, and `SECURITY.md`. Replace it everywhere:

```bash
grep -rl 'dpuse-connector-microsoft-onedrive' --exclude-dir=node_modules --exclude-dir=.git -Z . | xargs -0 sed -i '' 's/dpuse-connector-microsoft-onedrive/dpuse-connector-<name>/g'
```
